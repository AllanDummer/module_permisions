 ...
 * 
 * Added a check to see if the admin menu module is loaded on the permissions page
 */
Drupal.behaviors.adminMenuPermissionsSetupHelp = {
  attach: function (context, settings) {
    $('#permissions', context).once('admin-menu-permissions-setup', function () {
      ...
      // Added this check to prevent a jQuery exeption because these elements may not be present
      if ($(this).find('input[name$="[access administration pages]"]').length &&
    		  $(this).find('input[name$="[access administration menu]"]').length) {
